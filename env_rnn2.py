...
class GridEnvironment:
    ...
    def get_PEV(self, soc, actions):
        ...
        timestep = (10/60)  # 1 minute
        max_power = (11*100*3)/3500  # Maximum power in kW, attempting to scale 100 EVs?
        battery_capacity = (50*100*3)/3500 # Battery capacity, scaled by entire system, each represents 100 EVs, testing only having 1 mega EV
...
#NEED TO FOCUS ON THE SEQUENCE OF, OBSERVE STATE, CALCULATE ACTION, CALCULATE REWARD etc
    ...
    def calculate_reward(self, next_P_EV, action):
        ...
        #test
        #reward= -1*np.abs(current_demand- (current_solar + current_wind + np.sum(next_P_EV)))
